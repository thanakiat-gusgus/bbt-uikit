import React from "react";
import Text from "../Text/Text";
import Link from "../Link/Link";
import LogoIcon from "../Svg/Icons/Logo";
import KubIcon from "../Svg/Icons/KubIcon";
import BreadCrumbs from "./Breadcrumbs";

export default {
  title: "Components/Breadcrumbs",
  component: BreadCrumbs,
  argTypes: {},
};

export const Default: React.FC = () => {
  return (
    <Text p="32px">
      <BreadCrumbs mb="32px">
        <Link href="/" color="bbtColor" style={{ fontWeight: 400 }}>
          Link
        </Link>
        <Text color="textDisabled">Crumb 1</Text>
        <Text color="textDisabled">Crumb 2</Text>
      </BreadCrumbs>
      <BreadCrumbs>
        <Text>BBT-Swap</Text>
        <Text>The #1 AMM and yield farm on Bitkub Smart Chain.</Text>
      </BreadCrumbs>
    </Text>
  );
};

export const CustomSeparator: React.FC = () => {
  return (
    <Text p="32px">
      <Text mb="16px">
        <BreadCrumbs separator={<LogoIcon width="24px" />}>
          <Link href="/" color="bbtColor" style={{ fontWeight: 400 }}>
            Link
          </Link>
          <Text color="textBitkub">Crumb 1</Text>
          <Text color="textBitkub">Crumb 2</Text>
        </BreadCrumbs>
      </Text>
      <Text mb="16px">
        <BreadCrumbs separator={<KubIcon width="48px" />}>
          <Link href="/" color="failure" style={{ fontWeight: 400 }}>
            Link
          </Link>
          <Link href="/" color="primary" style={{ fontWeight: 400 }}>
            Link 2
          </Link>
          <Text color="textBitkub">Crumb 2</Text>
        </BreadCrumbs>
      </Text>
    </Text>
  );
};
